package com.bgaming.aztecclusters.service;
...
import com.bgaming.aztecclusters.entity.dto.SpinResponse;
import com.bgaming.aztecclusters.mapper.PlayerAdditionalInformationMapper;
...
public class PlayerServiceImpl implements IPlayerService {
        ...
        if (player != null) {
            ...
            try {
                if (player.getExtendJson().containsKey(SCENE)) {
                    ...
                    pai.setPlayTimes(player.getETimes());
    ...
    public void restorePlayerData(Player player, JSONObject gameInfo) {
        ...
        options.put("layout", LayoutData.builder().reels(COLUMNS).rows(ROWS).build());
        options.put("paytable", LotteryConfig.PAYTABLE);
        options.put("screen", LotteryConfig.SCREEN);
        options.put("lines_count", 25);
