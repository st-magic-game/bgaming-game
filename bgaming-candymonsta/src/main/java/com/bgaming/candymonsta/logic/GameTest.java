...
import static com.bgaming.candymonsta.config.LotteryConfig.REQUEST_TYPE;
...
//        player.getExtendJson().put("betType",1);
        ...
        for (int i = 0; i < betCount; i++) {
            try {
                ...
                while (lastScene.getOpenFreeNum() > 0 || (lastScene.getType() == 1 && lastScene.getFreeNum() > 1)){
                    player.getExtendJson().put(REQUEST_TYPE,1);
                    table.codeResultData(player, betScore, factor);
                    scenes = getScenes(player);
                    lastScene = scenes.get(scenes.size() - 1);
                    hasFree = true;
                }
                player.getExtendJson().put(REQUEST_TYPE,0);
//                SpinResponse response = table.generateResponse(scenes, false, player.getUser().getScore());

                double winTemp = scenes.stream().map(Scene::getGold).reduce(Double::sum).get();
                if (hasFree) {
                    int size = lastScene.getHoldWildIndexes().size();
                    Integer count = wildSize.getOrDefault(size, 0);
                    wildSize.put(size,count+1);
                    freeWin += winTemp;
                    bingoFreeCount++;
                } else {
                    normalWin += winTemp;
