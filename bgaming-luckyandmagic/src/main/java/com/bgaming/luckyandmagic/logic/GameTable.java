package com.bgaming.luckyandmagic.logic;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.bgaming.luckyandmagic.config.LotteryConfig;
import com.bgaming.luckyandmagic.entity.BonusData;
import com.bgaming.luckyandmagic.entity.PrizeIcon;
import com.bgaming.luckyandmagic.utils.DateTimeUtil;
import com.bgaming.luckyandmagic.entity.dto.OutCome;
import com.bgaming.luckyandmagic.entity.dto.RoundDetailDto;
import com.bgaming.luckyandmagic.entity.dto.SpinResponse;
import com.bgaming.luckyandmagic.entity.Scene;
...
public class GameTable extends TableSink {
    ...
    private RoundDetailDto generateRoundDetail(SpinResponse response, double beforeScore, Player player, Scene scene) {
        ...
        BigDecimal orderStake = DecimalUtil.getBigDecimal2(scene.getBetScore() * LotteryConfig.SUB_UNITS);
        BigDecimal realWin = DecimalUtil.getBigDecimal2(response.getOutcome().getWin().doubleValue() / LotteryConfig.SUB_UNITS);
        BigDecimal realProfit = DecimalUtil.getBigDecimal2(response.getOutcome().getWin().subtract(orderStake).doubleValue() / LotteryConfig.SUB_UNITS);
    ...
    private static final String[] SYMBOL_NAME = {"h1", "h2", "l1", "l2", "l3", "l4", "l5", "l6", "bonus"};
    ...
    private Object scatterIndex(Set<Integer> prizeIndex) {
        int[][] scatterIndexes = new int[3][2];
        for (Integer index : prizeIndex) {
            int[] idx = new int[2];
            idx[0] = index % LotteryConfig.COLUMNS;
            idx[1] = index / LotteryConfig.COLUMNS;
            scatterIndexes[index % LotteryConfig.COLUMNS] = idx;
        }
    ...
    private List<Scene> getResultScene(double betScore, double factor, Player gamePlayer) {
        ...
        if (scatterIndexes.size() >= 3) {
            BonusData bonusData = generateBonusData(factor);
            PrizeIcon scatter = new PrizeIcon();
            scatter.setIcon(LotteryConfig.SCATTER);
            scatter.setGold(DecimalUtil.getBigDecimal2(betScore * bonusData.getMultiplier()));
    ...
    private BonusData generateBonusData(double factor) {
        BonusData bonusData = new BonusData();
        int scatterBonusMul = scatterMulLineFactor(factor);
        bonusData.setMultiplier(scatterBonusMul);
        return bonusData;
    }
    ...
    private static void setScatterIcon(int[][] rotary, int scatterSize) {
        ...
        List<Integer> columns = new ArrayList<>(Arrays.asList(0, 1, 2));
        for (int i = 0; i < scatterSize; i++) {
            Integer index = columns.remove(RandomUtil.nextInt(columns.size()));
            rotary[RandomUtil.nextInt(LotteryConfig.ROWS)][index] = LotteryConfig.SCATTER;
        }
    }
