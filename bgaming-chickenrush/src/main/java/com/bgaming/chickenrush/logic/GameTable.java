...
import static com.bgaming.chickenrush.logic.ChickenRushContext.SUB_UNITS;
import static com.game.base.common.constant.GameKey.*;

@Slf4j
public class GameTable extends TableSink {

    private List<ApiClientResult> apiClientResults;
    ...
    public double getWinGold() {
        ...
        apiClientResults.forEach(r -> totalWin.updateAndGet(v -> v + r.getOutcome().getWin().doubleValue()));
        return totalWin.get()/ ChickenRushContext.SUB_UNITS;
    }
    ...
    public void codeResultData(Player player, double betScore, double factor, boolean bonus_buy, int purchased_feature_level,double realStake) {
        String pOrder = nextId();
        apiClientResults = ChickenRushContext.generateApiResult(player, betScore, factor, bonus_buy, purchased_feature_level, pOrder, realStake);
        result = apiClientResults.get(0);
    }
    ...
    private boolean checkInFree(Player player) {
        if (player.extendDataContainsKey("freeNum") && player.getEFreeNum() > 0) {
            ...
            log.info("玩家：{} 当前在免费场中，剩余免费次数： {}",player.getUser().getUserID(),freeNum);
            return true;
        }
        return false;
    }
    ...
    public Object startGame(Player player, String data) {
        try {
            ...
            if (options != null && !options.isEmpty()) {
                ...
                bonus_buy = options.containsKey("purchased_feature") && options.getString("purchased_feature").equals("bonus_buy");
                if (bonus_buy) {
                    purchased_feature_level = options.getInteger("purchased_feature_level");
                    if (purchased_feature_level < 0 || purchased_feature_level > 2) {
                        log.error("userid = {},作弊，玩家篡改客户端，等级错误",userId);
                        return null;
                    }
                }
            } else {
                ...
                int freeNum = player.getEFreeNum();
                if (apiClientResults == null || apiClientResults.isEmpty()) {
                    apiClientResults = player.getExtendDataList("apiClient",ApiClientResult.class);
                }
                result = apiClientResults.get(apiClientResults.size() - freeNum);
                player.setEFreeNum(freeNum - 1);
                sendServerMsg(player, player.getUser().getScore(), 0, null, getExtendData(result.getFlow().getPurchased_feature().containsKey("name")));
                return JSONObject.parseObject(JSONObject.toJSONString(result));
            }
            stake = DecimalUtil.getBigDecimal2(stake).doubleValue();
            orderStake = DecimalUtil.getBigDecimal2(stake / ChickenRushContext.SUB_UNITS).doubleValue();
            if (cheatingDetection(player, stake)) return null;
            realStake = DecimalUtil.getBigDecimal2(stake / ChickenRushContext.SUB_UNITS).doubleValue();
            if (bonus_buy) {
                if (purchased_feature_level == 0) {
                    realStake = DecimalUtil.getBigDecimal2(stake * 50 / ChickenRushContext.SUB_UNITS).doubleValue();
                } else if (purchased_feature_level == 1) {
                    realStake = DecimalUtil.getBigDecimal2(stake * 100 / ChickenRushContext.SUB_UNITS).doubleValue();
                } else {
                    realStake = DecimalUtil.getBigDecimal2(stake * 300 / ChickenRushContext.SUB_UNITS).doubleValue();
                }
            }
            ...
            do {
                ...
                this.codeResultData(player, stake, factor,bonus_buy,purchased_feature_level,realStake);
    ...
    private void sendServerMsg(Player player, double beforeScore, double betScore, UsePrize u, JSONObject extData) {
        ...
        if (finish) {
            beforeScore = player.getUser().getScore();
    ...
    private void sendDataLog(Player player,double stockScore) {
        ...
        JSONObject jsonObject = new JSONObject();
        List<ApiClientResult> apiClientResults = player.getExtendDataList("apiClient",ApiClientResult.class);
        jsonObject.put(ICON_DATA, JSONObject.toJSONString(ChickenRushContext.generateRoundDetail(apiClientResults,beforeScore,player,realStake)));
