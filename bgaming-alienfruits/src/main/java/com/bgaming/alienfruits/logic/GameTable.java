...
import com.bgaming.alienfruits.entity.client.Balance;
...
public class GameTable extends TableSink {
    ...
    public void codeResultData(Player player, double betScore, double factor, boolean bonus_buy,boolean inFree) {
        String pOrder = nextId();
        List<ApiClientResult> apiClientResults = AlienFruitsContext.generateApiResult(player, betScore, factor, bonus_buy, pOrder);
    ...
    public Object startGame(Player player, String data) {
        try {
            ...
            do {
                ...
                this.codeResultData(player, stake, factor,bonus_buy,inFree);
    ...
    private void sendDataLog(Player player,double stockScore) {
        ...
        JSONObject jsonObject = new JSONObject();
        List<ApiClientResult> apiClientResults = player.getExtendDataList("apiClient",ApiClientResult.class);
        jsonObject.put(ICON_DATA, JSONObject.toJSONString(AlienFruitsContext.generateRoundDetail(apiClientResults,beforeScore,player,realStake)));
