package com.bgaming.alienfruits.logic;


import com.alibaba.fastjson.JSONObject;
import com.bgaming.alienfruits.config.PayTableConfig;
import com.bgaming.alienfruits.config.SymbolConfig;
import com.bgaming.alienfruits.entity.PayTable;
import com.bgaming.alienfruits.entity.Symbol;
import com.bgaming.alienfruits.entity.client.ApiClientResult;
import com.bgaming.alienfruits.entity.client.Flow;
import com.bgaming.alienfruits.entity.client.Outcome;
import com.bgaming.alienfruits.entity.client.SpecialSymbols;
import com.bgaming.alienfruits.entity.log.RoundDetailDto;
import com.bgaming.alienfruits.config.GlobalConfig;
import com.bgaming.alienfruits.utils.DateTimeUtil;
...
@Component
public class AlienFruitsContext {
    ...
    public AlienFruitsContext(SymbolConfig config, PayTableConfig payTableConfig, GlobalConfig globalConfig) {
    ...
    private static int getMultiplier() {
        ...
        if (random <= GLOBAL_CONFIG.getMultiplierPro().get(2).doubleValue()) {
            ...
            return RandomUtil.nextInt(21,31);
        } else if (random <= GLOBAL_CONFIG.getMultiplierPro().get(0).doubleValue()) {
            return RandomUtil.nextInt(11,21);
        }
        return RandomUtil.nextInt(2,10);
    }
    ...
    private static void setSymbols(List<List<Integer>> reelTable,List<int[]> usePoint,double v,boolean spin) {
        ...
        if (RandomUtil.nextDouble() <= GLOBAL_CONFIG.getPrizePro().doubleValue() * Math.pow(v,3) && spin) {
            ...
            if (RandomUtil.nextDouble() <= 0.003) {
                num = 12;
            }
            map.put(symbol.getIndex(),num);
            for (int i = 0; i < num; i++) {
        ...
        for (int i = 0; i < size; i++) {
            int[] position = usePoint.remove(0);
            map.forEach((k,value) -> {
                if (value >= 9 && RandomUtil.nextDouble() <= (0 / Math.pow(v,3))) {
    ...
    private static void generateOutcome(ApiClientResult clientResult, double stake, double v, boolean bonus_buy, boolean isFree) {
    ...
    public static List<ApiClientResult> generateApiResult(Player player,double stake, double v,boolean bonus_buy,String pOrder) {
        ...
        generateOutcome(clientResult,stake,v,bonus_buy && totalFreeNum == 0,isFree);
